...
const IconWalletAvatar = () => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" aria-hidden="true">
    <path d="M8.2 6.6V6a3.4 3.4 0 0 1 3.4-3.4h.8A3.4 3.4 0 0 1 15.8 6v.6" stroke="#fff" strokeWidth="2.2" strokeLinecap="round" />
    <rect x="3" y="6.6" width="18" height="13.4" rx="3.8" fill="#fff" />
    <circle cx="16.6" cy="13.3" r="1.7" fill="#4343ee" />
  </svg>
);
...
const IconScan = () => (
  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.1" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
    ...
    <rect x="8.2" y="8.2" width="2.8" height="2.8" rx="0.7" fill="currentColor" stroke="none" />
    <rect x="13" y="13" width="2.8" height="2.8" rx="0.7" fill="currentColor" stroke="none" />
  </svg>
);
...
const IconChevronRight = () => (
  <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3.1" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
    <path d="m9 5 7 7-7 7" />
  </svg>
);
...
const START_ACTIONS = [
  ...
  { key: 'deposit', lines: ['Deposit from', 'Binance'], icon: <span className="tw-bnb-glyph"><IconBnb /></span> },
...
function MarketIcon({ symbol, imageUrl }) {
  ...
  if (symbol === 'BNB') return <span className="tw-mkt-icon tw-mkt-bnb"><IconBnb size={28} /></span>;
  ...
  const marketRows = useMemo(() => {
    const rows = (trust.tokens || []).map((token) => {
      ...
      return {
        ...
        symbol: token.symbol,
        name: SYMBOL_NAMES[token.symbol] || token.name || token.symbol,
        price: quote?.usd ?? 0,
